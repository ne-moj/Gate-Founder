...
**Options:**
  ...
  --radius <value>   : Filter gates within radius.
  --sector           : Filter gates in current sector.
...
**Examples:**
  ...
  /gate admin list --radius 50 --nearest : List nearest gates within radius
]]
        end
        ...
        for i, arg in ipairs(adminArgs) do
            if arg == "--owner" then filterOwner = tonumber(adminArgs[i+1]) end
            if arg == "--nearest" then showNearest = true end
            if arg == "--sector" then showSector = true end
            if arg == "--radius" then radius = tonumber(adminArgs[i+1]) end
        end
